clear;
[fnm pth]=uigetfile('*.tif','Select registerated Calcium image');
 %% load file
im_inform=imfinfo([pth fnm]);
for i=1:numel(im_inform)
    Y(:,:,i)=imread([pth fnm],i);
end
Y = Y - min(Y(:)); 
...
tau = 30;                                          % std of gaussian kernel (half size of neuron) 
pp = 2;
...
options.save_avi=0;
 options.make_avi=0;
save([pth fnm(1:end-4) '_astro_rst.mat'],'C_df','Yr','A_or','C_or','b2','f2','Cn','Coor','options','json_file','-v7.3');
% save([pth fnm(1:end-4) '_astro_rst_compressed.mat'],'C_df','A_or','C_or','b2','f2','Cn','Coor','options','json_file','-v7.3');
if options.save_avi
