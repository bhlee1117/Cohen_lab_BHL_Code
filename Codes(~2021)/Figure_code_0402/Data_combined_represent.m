...
xtick={'HC','CFC','Ret 1','Ret 2','Ret 3','Ret4'}; color=[.2 .8 .8; .2 .2 .8;.8 .2 .8 ;.8 .5 .8];
xtick2={'A          B','A           B'};
...
plot_TXN(fracTXN,[1 2],0,color,N,xtick,25)
...
group=5; class=1; ch_class=1; %1: Overlap ch, 2: Reactivation Ch, Reactivation Neg.
groups=[1 2];
color=[0.2 0.8 0.8; 0.2 0.3 0.3];
...
plot_cmpr_int(compare_ch,[2 4;3 5;4 6],color,class,0,12,{'C{\bf\cap}R2','R1{\bf\cap}R3','R2{\bf\cap}R4','R3{\bf\cap}R4'},1)  
% 1:sw, 10: y_lim, Chance
%% Cumulative fraction
color=[0.2 0.8 0.8; 0.8 0.2 0.8]; group=2; session=[2 5]; groups=[1 3];
...
[p lifetime]=plot_cum_frac(cum_mat,groups,session,color,N,xtick,40,3,0); % value p : 1st column = ttest, 2nd column =MW, 3rd = KS, fitline =1, true
...
plot_venn(omitna_data,[1],[2],[3 4 5],N,tick_coord) % group, cond, session of interest
...
%[dscm tmp_dat]=plot_discrimination(Freeze,cond_mat,groups,{[2 3;2 4;2 5;2 6],[2 3;2 4;2 5;2 6]},1,N);
cc=plot_frz_cum(Freeze,cum_mat,groups,session)
%% Plot tree plot
group=6;
xtick={'+','++','+++','++++','+++++'}; %group=[5 6]; interest=[2 6 14 30 62];
tree=plot_tree(omitna_data,N,2,group); % 1:overlap, 2:Conditional; group