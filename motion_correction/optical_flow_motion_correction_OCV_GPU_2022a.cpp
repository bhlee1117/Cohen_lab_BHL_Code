...
//     Ptr<cuda::DensePyrLKOpticalFlow> OF = cuda::DensePyrLKOpticalFlow::create();
    ...
    Ptr<cv::cuda::GpuMat> gpuMov;
    gpuMov->upload(ocvMxArrayToMat_single(prhs[1])); // Convert gpuArray into cv::gpu::GpuMat 

    Ptr<cv::cuda::GpuMat> gpuMov_template;
    gpuMov_template->upload(ocvMxArrayToMat_single(prhs[0])); // Convert gpuArray into cv::gpu::GpuMat 

    cuda::GpuMat gFlow(gpuMov->size(),CV_32FC2);
    
    OF->calc(*gpuMov,*gpuMov_template,gFlow);
    
    plhs[0] = ocvMxArrayFromMat_single(download(gFlow.reshape(1)));
//     gFlow.release();
}
