function alp_patterns = dragonflySimplePatterns_yq(liveOffset,liveArea,cell_rois,dmd,registration_data) % hadamard_mode)
%
%   2021 Yitong Qi
...
%
    ...
    hold(gca,'on');
    title('Please clicky DMD pattern')
    npts = 1;
    nroi = 1;
    while(npts > 0)
    ...
    pat_out = in{1};
    if length(in)> 1
        for i_pat = 2:length(in)
            pat_out = cat(3,pat_out,in{i_pat});
        end
    end
