...
bool imgThreadFlag;
bool procThreadFlag;
bool histoThreadFlag;

uint16_t lastImageData[IMG_WIDTH * IMG_HEIGHT];
uint16_t lastImageDataWidth;
uint16_t lastImageDataHeight;
...
std::thread histoThread;
// mutex handles
std::mutex ghMutexImg; 
std::mutex ghMutexLims; 
std::mutex ghMutexHisto; 
std::mutex ghMutexFrame; 
std::mutex ghMutexCapturing; 
...
// 	int step_h = (int)(((float)myImageDataHeight-1)/(IMG_HEIGHT/4) + 1);
    int step_w = (int)(((float)myImageDataWidth -1)/HISTO_SIZE + 1);
	int step_h = (int)(((float)myImageDataHeight-1)/HISTO_SIZE + 1);
...
//                 float fval = (float)myImageData[ix * myImageDataHeight + iy];
...
//                 workImgSurfBuffer[(ix+tox) * IMG_HEIGHT + (iy+toy)] = (0xFF << 24) | (val << 16) | (val << 8) | val;
			}
		}
// 	ReleaseMutex(ghMutexLims);
    ghMutexLims.unlock();
...
// 	SDL_RenderClear(renderer); // blank the rendering
    SDL_FillRect(window_surface, NULL, SDL_MapRGBA(window_surface->format,64, 64, 64, 255));
...
// 		SDL_DestroyTexture(myImgTex); // free texture
        ...
        ghMutexImg.lock();
//             SDL_BlitSurface(myReadyImgSurf,&imgSrcRect,window_surface,&imgDestRect);
            SDL_BlitScaled(myReadyImgSurf,&imgSrcRect,window_surface,&imgDestRect);
        ghMutexImg.unlock();
...
// 		SDL_DestroyTexture(myHistoTex); // free texture
        ghMutexHisto.lock();
//             SDL_BlitSurface(myReadyHistoSurf,&histoSrcRect,window_surface,&histoDestRect);
            SDL_BlitScaled(myReadyHistoSurf,&histoSrcRect,window_surface,&histoDestRect);
        ghMutexHisto.unlock();
...
void procThreadFcn() {
    ...
    procThreadFlag = false;
// 	return 0;
}

void histoThreadFcn() {
    histoThreadFlag = true;
	while ( continueRunningAllThreads ) {
...
// 	hHistoThread = (HANDLE)_beginthreadex( NULL, 0, &histoThreadFcn, NULL, 0, &threadID );
    ...
    imgThread = std::thread(imgThreadFcn);
    imgThread.detach();
    
    procThread = std::thread(procThreadFcn);
    procThread.detach();
    
    histoThread = std::thread(histoThreadFcn);
    histoThread.detach();
}

bool close_SDL_window_and_threads(){
...
// 	SDL_FreeSurface(myWork1ImgSurf  ); 
// //     myWork1ImgSurf   = NULL;
// 	SDL_FreeSurface(myWork2ImgSurf  ); 
// //     myWork2ImgSurf   = NULL;
// 	SDL_FreeSurface(myWork1HistoSurf); 
// //     myWork1HistoSurf = NULL;
// 	SDL_FreeSurface(myWork2HistoSurf); 
// //     myWork2HistoSurf = NULL;
//     SDL_DestroyWindow(window);
//     SDL_Event e;
//     while (SDL_PollEvent(&e)) {
//         if (e.type == SDL_QUIT) return true;
//     }
    while(procThreadFlag||histoThreadFlag||imgThreadFlag){SDL_Delay(2);}
