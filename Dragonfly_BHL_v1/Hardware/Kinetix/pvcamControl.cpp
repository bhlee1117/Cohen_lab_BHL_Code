...
bool pvcam_startup(void) {
	if (PV_OK == InitAndOpenOneCamera(contexts, cSingleCamIndex)){
		...
		pl_cam_register_callback_ex3(cam_ctx->hcam, PL_CALLBACK_EOF, (void*)GenericEofHandler, cam_ctx);
...
//         output_speedtable(out_speedtable);
        ...
        pl_set_param(cam_ctx->hcam, PARAM_READOUT_PORT, (void*) &cam_ctx->speedTable[2].value);
        pixel_filter_off();
        return true;
	}
...
void output_pp_parameter_state(std::string &out_msg){
    char s_buf[100];
    out_msg.clear();
    const std::vector<PvcamPpFeature> ppFeatures = DiscoverCameraPostProcessing(cam_ctx);
    ...
    for (const PvcamPpFeature& ppFeature : ppFeatures)
    {
        ...
        for (const PvcamPpParameter& ppParam : ppFeature.parameterList)
        {
            ...
            pl_get_param(cam_ctx->hcam,PARAM_PP_PARAM,ATTR_CURRENT,(void*) &cur_val);
...
bool aq_live_restart(bool isCentered, double *inputROI, double binning, double exposureTimeMiliseconds) {
	if(!cam_ctx->isCamOpen){
		printf("no camera open\n");
		return false;
	}
...
// 	
//     NVPC scanModes,exposeOutModes;
//     ReadEnumeration(cam_ctx->hcam, &scanModes, PARAM_SCAN_MODE, "PARAM_SCAN_MODE");
//     ReadEnumeration(cam_ctx->hcam, &exposeOutModes, PARAM_EXPOSE_OUT_MODE,"PARAM_EXPOSE_OUT_MODE");
    
//     const int32 newScanMode = PL_SCAN_MODE_PROGRAMMABLE_LINE_DELAY;
//     pl_set_param(cam_ctx->hcam, PARAM_SCAN_MODE, (void*)&newScanMode);
//     
//     const uns16 newScanLineDelay = 1;
//     pl_set_param(cam_ctx->hcam, PARAM_SCAN_LINE_DELAY,(void*)&newScanLineDelay);
        
    
    
//     int32 newScanDirection = PL_SCAN_DIRECTION_DOWN_UP;
//     pl_set_param(cam_ctx->hcam, PARAM_SCAN_DIRECTION,(void*)&newScanDirection);
//     
//     rs_bool newScanDirectionRst = FALSE;
//     pl_set_param(cam_ctx->hcam, PARAM_SCAN_DIRECTION_RESET,(void*)&newScanDirectionRst);
    
// 	const int16 expMode = EXT_TRIG_INTERNAL | EXPOSE_OUT_LINE_TRIGGER;
    const int16 expMode = EXT_TRIG_INTERNAL;
//     SelectCameraExpMode(cam_ctx, expMode, TIMED_MODE, EXT_TRIG_INTERNAL);
//     rgn_type roi = {0,2048,1,0,2048,1};
	rgn_type roi = {(uns16)(vPos-1), (uns16)(vPos+vSize-2), (uns16)vBinning,
                    (uns16)(hPos-1), (uns16)(hPos+hSize-2), (uns16)hBinning};
// 	
	if (PV_OK != pl_exp_setup_cont(cam_ctx->hcam, 1, &roi, expMode,
                exposureTimeMicroseconds, &exposureBytes, bufferMode)){
        return false;
	}
                    
    exposureTimeSeconds = double(exposureTimeMicroseconds)/1e6;
    pl_get_param(cam_ctx->hcam, PARAM_EXPOSURE_TIME, ATTR_CURRENT, &readoutTimeMicroseconds);
...
// 
    ghMutexCapturing.lock();
        ...
        if (PV_OK != pl_exp_start_cont(cam_ctx->hcam, liveCircBufferInMemory, circBufferBytes))
        {
...
//     SelectCameraExpMode(cam_ctx, expMode, STROBED_MODE, (void*) &expMode);
	...
	if (PV_OK == pl_exp_setup_cont(cam_ctx->hcam, 1, &roi, expMode,
		...
		exposureTimeSeconds = double(exposureTimeMicroseconds)/1e6;
		pl_get_param(cam_ctx->hcam, PARAM_READOUT_TIME, ATTR_CURRENT, &readoutTimeMicroseconds);
		readoutTimeSeconds = double(readoutTimeMicroseconds)/1e6;
        printf("Live acquisition set up success on camera\n");
	}
...
void aq_sync_start(int32 recordFrames, const char *fpath, const char *fext) {
    std::thread aq_sync_start_thread = std::thread(aq_sync_start_Fcn, recordFrames, fpath, fext);
...
//     const uns32 circBufferBytes = (MAX_BUFFER_SIZE<circBufferBytes_test)?MAX_BUFFER_SIZE:circBufferBytes_test;
    const uns32 circBufferBytes = (MAX_BUFFER_FRAMES< (uns32)recordFrames)?MAX_BUFFER_SIZE:circBufferBytes_test;
    ...
    if (PV_OK != pl_exp_start_cont(cam_ctx->hcam, circBufferInMemory, circBufferBytes))
    {
        PrintErrorMessage(pl_error_code(), "pl_exp_start_cont() error");
...
// 		if(PV_OK != pl_exp_check_cont_status(cam_ctx->hcam, &status, &bytes_arrived, &buffer_cnt)){
//             delete [] circBufferInMemory;
//             ghMutexCapturing.lock();
//                 isCapturing = false;
//             ghMutexCapturing.unlock();
...
//     std::fwrite(reinterpret_cast<const void*>(circBufferInMemory), 1, circBufferBytes, f);
// //     fprintf(f,"%s","test");
//     std::fclose(f);
...
//     ghMutexCapturing.unlock();
//     delete [] circBufferInMemory;
    if(stream_data_to_file(fname_bin_tgt, circBufferInMemory, circBufferBytes, recordFrames)){
        delete [] circBufferInMemory;
    }
	return true;
}
...
//             bufframe.buf = liveCircBufferInMemory + circBufferCnt * exposureBytes;
...
//         if(PV_OK != pl_exp_get_latest_frame(cam_ctx->hcam,&(latest_frame))){
            bufframe.buf = nullptr;
            return &bufframe;
        }
//         bufframe.buf = cam_ctx->eofFrame;
...
bool stream_data_to_file(char* fpath, uns8* circBufferPtr, const uns32 circBufferSize, int totalFrames){
    ...
    while(status != READOUT_NOT_ACTIVE){
		if(PV_OK != pl_exp_check_cont_status_ex(cam_ctx->hcam, &status, &bytes_arrived, &buffer_cnt, &frame_info)){
...
//         std::fwrite(reinterpret_cast<const void*>(circBufferPtr + bytesWritten), 1,
//                     bytesToWrite, f);
//         if(bytesToWrite+bytesWritten < circBufferSize){
        std::fwrite(reinterpret_cast<const void*>(circBufferPtr + bytesBufferInc), 1,
                    bytesToWrite, f);
//         }
//         else{
//             std::fwrite(reinterpret_cast<const void*>(circBufferPtr + bytesBufferInc), 1,
...
//         bytesWritten %= circBufferSize;
        bytesBufferInc = bytesWritten % circBufferSize;
//         bytesToWrite = (totalFrames*exposureBytes-circBufferSize*buffer_cnt - bytesWritten);
        bytesToWrite = (totalFrames*exposureBytes - bytesWritten);
//         std::fwrite(reinterpret_cast<const void*>(circBufferPtr + bytesWritten), 1,
//                 bytesToWrite, f);
//         std::fwrite(reinterpret_cast<const void*>(circBufferPtr + bytesBufferInc), 1,
//                     bytesToWrite, f);
        std::fwrite(reinterpret_cast<const void*>(circBufferPtr+circBufferSize-bytesToWrite), 1,
                    bytesToWrite, f);
    }
    std::fclose(f);   
    return true;
