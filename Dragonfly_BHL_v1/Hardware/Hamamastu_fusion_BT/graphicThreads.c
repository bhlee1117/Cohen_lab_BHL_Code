...
unsigned __stdcall imgThreadFcn( void* pArguments ) {
	SDL_Event event;
	bool done = false;
	static DCAMBUF_FRAME *newFrame = NULL;
	static uint16_t myImageData[IMG_WIDTH*IMG_HEIGHT];
	static int myImageDataWidth = IMG_WIDTH;
	static int myImageDataHeight = IMG_HEIGHT;
...
unsigned __stdcall convertThreadFcn( void* pArguments ) {
				char dcimg_fname[MAX_MSG_LEN], fname_bin_temp[MAX_MSG_LEN], fname_bin_tgt[MAX_MSG_LEN];
				sprintf(dcimg_fname,"%s.%s",fpath_dcimg_copy,fext_dcimg_copy);
				rm_dup_slashes(dcimg_fname,dcimg_fname);
				sprintf(fname_bin_temp,"%s.%s",fpath_bin_temp_copy,fext_bin_temp_copy);
				rm_dup_slashes(fname_bin_temp,fname_bin_temp);
				sprintf(fname_bin_tgt,"%s.%s",fpath_bin_tgt_copy,fext_bin_tgt_copy);
				rm_dup_slashes(fname_bin_tgt,fname_bin_tgt);
				if (convert_file(dcimg_fname,fname_bin_temp)){
					// successful, lets delete dcimg file
					remove(dcimg_fname);
					MoveFileEx(fname_bin_temp,fname_bin_tgt,MOVEFILE_REPLACE_EXISTING);
				}
	_endthreadex( 0 );
	return 0;
}
...
// void __mexFunction__graphicThreads( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ){
