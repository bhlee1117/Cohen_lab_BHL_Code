...
switch disp_img
    case 'yes'
        ...
        for j = 1:nroi;
            ...
            if size(roi,2)==3
                bg_mask = xor(inpolygon(x,y,roi{2}(:,1),roi{2}(:,2)),inpolygon(x,y,roi{3}(:,1),roi{3}(:,2)));
            else
                 se1 = strel('disk',round(sqrt(length(find(inpoly)))*.5),8);
                  ...
                  roi(j,2:3)= cellfun(@(x) flip(x,2),bwboundaries(bg_mask),'uniformoutput',false);
            end
...
%             bg_mask = xor(mask,inpoly); 
            ...
            cellfun(@(x) plot(x(:,1), x(:,2), 'Linewidth', 1,'Color',currcolor),roi);
