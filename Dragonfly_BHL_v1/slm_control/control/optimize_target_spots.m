...
ostep_scale = [.1 .1 1]*2; % search space dimensions for [x y z]
...
%         delta_ostep(it_spot,it_coeff) = -p(2)/p(1)/2; % location of max of parabola
        ...
        if range(polyval(p,osteps_interp))/(mean(polyval(p,osteps_interp))-100)<.01
            delta_ostep(it_spot,it_coeff) = 0;
        end
          
        subplot(size(current_xyz,1),3,(it_spot-1)*3+it_coeff)
