...
% 	Target=1-double(Target);
    ...
    subplot(2,2,3);imshow(abs(Target),[]);subplot(2,2,1);imshow(input_intensity,[]);colormap(gca,jet)
...
for i=1:iteration_num
  B = abs(input_intensity) .* (A+eps)./(abs(A)+eps);%
%   B = abs(input_intensity) .* exp(1i*angle(A));
  C = fftshift(fft2(B));

  D = (m*abs(Target_sqrt.*s_mask) - (1-m)* abs(C).*f_mask).*(C+eps)./(abs(C)+eps);%
...
imwrite(pic_fft_out,'C:\Program Files\Meadowlark Optics\Blink 1920 HDMI\Image Files\square_2x1_MRAF.bmp')