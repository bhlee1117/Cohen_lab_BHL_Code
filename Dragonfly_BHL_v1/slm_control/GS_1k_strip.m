...
%      Target =exp(-((X-x0).^2 + (Y-y0).^2)./(2*10^2));
     m=5;
     n=8;
%     m=3; %512x512
%     n=2;  %512x512
...
%     n=3;  %512x512
    ...
    Target(Target<.6)=0;
    Target(Target>0)=1;
%     figure;imshow2(Target,[]);colorbar
    
%     Target = imgaussfilt(Target,10);
%     Target(Target>0)=1;
...
%     A = angle(A).*slm_mask;
    ...
    [X,Y] = meshgrid(x,y);
        R = 1.15e-3; a = .6; %512x512 circ
%     R = 1.3e-3; a = .62; %512x512
%     R = .9e-3; a = .7; %324x324
%     R = 1.65e-3; a = .8; %1k strip
    A = 4*R*(a*X.^2+(1-a)*Y.^2);
...
% hSLMApp.current_mask = pic_fft_out;
% pic_fft_out = uint8(mat2gray(pic_fft_out)*255);
% figure(100);clf;imshow(pic_fft_uint8,[])
%%
save('D:\Code\Dragonfly_yq_v1\slm_control\1024x1024.mat','pic_fft_out')
