...
if isGPU
    wt = gpuArray.ones(1,1,length(x))/numel(x);
...
%     sptField = exp(1i*(-slmPh+sptPh));
%     sptField = input_intensity.* sptField;
%     sptField = sum(sptField,[1 2])/numel(xj);
    sptField = sum(input_intensity.*exp(1i*(-slmPh+sptPhOffset)),[1 2])*d^2;
%     sptField = sum(input_intensity.*exp(1i*(-slmPh+sptPh)),[1 2])*d^2;
%     sptField = sum(input_intensity.*exp(1i*(-slmPh+sptPh)),[1 2])*d^2;
%     wt = mean(abs(sptField))/abs(sptField).*wt;

    sptPhOffset = angle(sptField);%./sptField;
%     slmField = exp(1i*(-sptPhOffset-sptPh));
%     slmField = slmField.*wt;
%     slmField = sum(slmField,3);
    slmField = sum(exp(1i*(-sptPhOffset-sptPh)).*wt.*input_intensity/4,3);
%     slmField = sum(exp(1i*(sptPhOffset+sptPh)).*wt.*input_intensity/4,3);    
