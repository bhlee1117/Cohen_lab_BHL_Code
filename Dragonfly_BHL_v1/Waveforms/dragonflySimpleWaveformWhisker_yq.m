function allWaves = dragonflySimpleWaveformWhisker_yq(df_app)%nCells,recordFrames,maxBlueIntensity,centeredROI,exposureTime)
%
...
w_cam = extend([wp0 w_cam_frame.repeat(n_frames)],t_total);  % extra trigger to initiate recording
w_gs = extend([wp0 w_gs_frame.repeat(n_gs_frames)],t_total);  % extra trigger to initiate recording

%% Electromagnet Waveform Def

w_stimWav = flexRampTrainWave(dt,t_total);
w_stimWav.tBefore = 5.95; % duration off 
w_stimWav.tRamp = 0.1; % duration on
w_stimWav.period = 12; % seconds

w_ch1On = flexRampTrainWave(dt,t_total);
w_ch1On.tBefore = w_stimWav.tBefore; % duration off 
w_ch1On.tRamp = w_stimWav.tRamp/2; % duration on
w_ch1On.period = w_stimWav.period; % seconds

w_ch2On = w_stimWav - w_ch1On;
...
w_out{6,1} = 'magnetNTrigger';
w_out{6,2} = w_ch2On;

w_out{7,1} = 'magnetPTrigger';
w_out{7,2} = w_ch1On;
% 
% w_out{4,1} = 'pololu';
