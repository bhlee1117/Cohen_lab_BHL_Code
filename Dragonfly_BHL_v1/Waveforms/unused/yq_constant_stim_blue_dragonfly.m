function allWaves = yq_constant_stim_blue_dragonfly(nCells,recordFrames,maxBlueIntensity,centeredROI,exposureTime)
if ~exist('nCells','var')
    nCells = [];
end
...
w_stim.tRamp = 3; % duration on
w_stim.period = 3; % seconds

w_lime = flexRampTrainWave(dt,t_total);
w_lime.tBefore = 0; % duration off
w_lime.tRamp = 0.5; % duration on
