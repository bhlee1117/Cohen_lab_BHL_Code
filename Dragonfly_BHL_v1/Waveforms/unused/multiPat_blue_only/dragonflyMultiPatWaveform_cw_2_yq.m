function allWaves = dragonflyMultiPatWaveform_cw_2_yq(df_app)%nCells,recordFrames,maxBlueIntensity,centeredROI,exposureTime)
%
...
t_dmd_frame         = floor((t_cam_frame-t_jit)/dt/n_pat)*dt; % expo time
...
w_stim_frame = [wp0 wp0.extend(t_dmd_frame-dt)+1];

w_cam_frame = wp1.extend(t_cam_frame);
w_dmd_frame = extend([wp0.extend(t_jit) repeat(wp1.extend(t_dmd_frame),n_pat)],t_cam_frame);
...
for ii=1:n_pat
    w_pat = [w_pat;
        ...
        repeat(extend(...
        [wp0.extend(t_jit) wp0.extend(t_dmd_frame*(ii-1)) w_stim_frame],...
...
w_stim1.tRamp = t_on; % duration on
w_stim1.period = t_total; % seconds

w_stim_c = flexRampTrainWave(dt,t_total);
w_stim_c.tBefore = 0.5; % duration off 
w_stim_c.tRamp = t_on_1; % duration on
w_stim_c.period = t_total; % seconds

w_stim2 = flexRampTrainWave(dt,t_total);
w_stim2.tBefore = 0.5 + t_on_1; % duration off 
w_stim2.tRamp = t_on - t_on_1; % duration on
w_stim2.period = t_total; % seconds
% w_stim2 = [baseWave(dt,0.35) w_stim2 baseWave(dt,0.25)];

w_stim = [w_stim1 + w_stim_c; w_stim2]*intens;
%------------end---------------------

%----3 constant stim sequence------
% w_stim1 = flexRampTrainWave(dt,t_total);
% w_stim1.tBefore = 0.5; % duration off 
...
% 
% w_stim2 = flexRampTrainWave(dt,t_total);
% w_stim2.tBefore = .5+t_on_1; % duration off 
% w_stim2.tRamp = t_on-t_on_1; % duration on
% w_stim2.period = t_total; % seconds
% % w_stim2 = [baseWave(dt,0.35) w_stim2 baseWave(dt,0.25)];
% 
...
% 
% w_stim = [w_stim1; w_stim2; w_stim3]*intens;
%------------end---------------------
