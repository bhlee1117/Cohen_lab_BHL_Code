function allWaves = dragonflyMultiPatWaveform_yq(df_app)%nCells,recordFrames,maxBlueIntensity,centeredROI,exposureTime)
%
...
t_exp_delay = 1e-5; %s trigger to exposure delay
% Collect app params
dt                  =   1/df_app.ni6343.rateConfigData; % DAQ clock rate
t_pulse_duration    =   dt;  % use dt max precision
t_cam_frame         =  df_app.ExposureTimeMsEditField_waveform.Value*1e-3 + t_jit + t_exp_delay; % s
...
t_dmd_frame_orange  = floor((t_cam_frame-t_jit*2)*orange_duty_cycle/dt)*dt; % expo time orange
t_dmd_frame_blue    = floor((t_cam_frame-t_jit*2)*blue_duty_cycle/(n_pat-1)/dt)*dt; % expo time blue
...
w_stim_frame_blue = [wp0 wp0.extend(t_dmd_frame_blue-dt*2)+1 wp0];
...
w_dmd = extend([wp0 wp0.extend(t_exp_delay) w_dmd_frame.repeat(n_frames)],t_total);
...
% w_stim1.tBefore = 0.45; % duration off 
% w_stim1.tRamp = .5; % duration on
% w_stim1.period = t_total; % seconds
% 
% w_stim2 = flexRampTrainWave(dt,t_total);
% w_stim2.tBefore = 0.65; % duration off 
% w_stim2.tRamp = 0.5; % duration on
...
w_stim1 = flexRampTrainWave(dt,t_total);
w_stim1.tBefore = 4; % duration off 
