...
if all(axPos(3:4)==roi(3:4))
    % Target image and ROI are in sync.
        persistent non_naive %#ok<TLEV>
        if ~isfield(handles.Image.UserData,'UpdateFlag')
            handles.Image.UserData.UpdateFlag = false;
        end
        if isempty(non_naive)
            try
                non_naive = true;
                disp 'callback set' 
                handles.Image.ButtonDownFcn = @ToggleButtonDownFcn;
                colorbar(handles.Image.Parent)
                get(uddobj)
            catch e
                disp(e) 
            end
        ...
        try
            if isequal(get(uddobj,'Running'),'off')
                img = getsnapshot(uddobj); 
                data = img;
            end
            if handles.Image.UserData.UpdateFlag
                maxdata = max(handles.Image.CData(:));
                if isfinite(maxdata) && maxdata
                    clims = [0 maxdata];
                else
                    clims = [0 255];
                end
                handles.Image.Parent.CLim = clims;
            else
            end            
        catch e
            disp(e)
...
function ToggleButtonDownFcn(hImage,hHit)
hImage.UserData.UpdateFlag = ~hImage.UserData.UpdateFlag;
if hImage.UserData.UpdateFlag
    disp 'now scale is live'
else
    disp 'now scale is static'
end
