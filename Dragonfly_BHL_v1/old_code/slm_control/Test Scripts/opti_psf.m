...
optimParamScale = [10 10 10];
nParams = numel(optimParamScale);
optimSteps = linspace(-1,1,4);
...
slmSettlingTime = 0.15; % s
src.ExposureTime = 0.01; % s
nLaserFrames = 10;
laserOnTime = src.ExposureTime*(nLaserFrames + 2)*s.Rate; % ms
vid.FramesPerTrigger = ceil((laserOnTime/1000 + slmSettlingTime)*nSteps/src.ExposureTime*2.5);
...
itParam = 2;
while itParam <= 2 % optimize one parameter each time
    for itStep = 1:nSteps
        test_xyz_offset = best_xyz_offset;
        test_xyz_offset(:,itParam) = test_xyz_offset(:,itParam) + optimParamScale(itParam).*optimSteps(itStep);
%         test_xyz_offset
        xyz = slmlocs - test_xyz_offset;
        ttd = [
...
%     agg_diffs = max(tovec(alldata(:,:,:,it_steps)));
    ...
    disp ' '
    disp(deltaOptim)
    best_xyz_offset(:,itParam) = best_xyz_offset(:,itParam) + deltaOptim(:,itParam).*optimParamScale(itParam)
    itParam = itParam + 1;
end
