function [mean_amplitudes std_amplitudes x_bin_centers indicies binnedData]=binning_data_median(Data,x_bin_edges)
% [mean_amplitudes std_amplitudes x_bin_centers indicies binnedData]=binning_data_median(Data,x_bin_edges)
...
for j = 1:length(x_bin_centers)
    ...
    if ~isempty(bin_values) 
        mean_amplitudes(j) = median(bin_values,'omitnan'); % Calculate mean and standard deviation if there are values in this bin
