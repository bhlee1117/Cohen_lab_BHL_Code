...
/* vparot 2012 */
int chklim(int idx, int MxN){
	return !(idx<0 || idx>=MxN);
}
	   ...
	   double *f_out)
{
  ...
  if (M % 2 == 0 && N % 2 == 0)
  {
	...
	index1 = (j * Mhalf + i);
	index2 = (2 * j * M + 2 * i);
	if(chklim(index1				,Mhalf*Nhalf)) ce = v[index1				];
	if(chklim(index1 - 1			,Mhalf*Nhalf)) no = v[index1 - 1			];
	if(chklim(index1 + 1			,Mhalf*Nhalf)) so = v[index1 + 1			];
	if(chklim(index1 - Mhalf		,Mhalf*Nhalf)) we = v[index1 - Mhalf		];
	if(chklim(index1 + Mhalf		,Mhalf*Nhalf)) ea = v[index1 + Mhalf		];
	if(chklim(index1 - Mhalf - 1	,Mhalf*Nhalf)) nw = v[index1 - Mhalf - 1	];
	if(chklim(index1 - Mhalf + 1	,Mhalf*Nhalf)) sw = v[index1 - Mhalf + 1	];
	if(chklim(index1 + Mhalf - 1	,Mhalf*Nhalf)) ne = v[index1 + Mhalf - 1	];
	if(chklim(index1 + Mhalf + 1	,Mhalf*Nhalf)) se = v[index1 + Mhalf + 1	];
	CE = index2;
	SO = index2 + 1;
	EA = index2 + M;
	SE = index2 + M + 1;
