...
corrimg = mean(dVmat.*imgs,3); %I guess this is a property of correlations. Multiplying signals together and summing will erase frequencies that do not vary together. 
corrimg = corrimg/mean(dV.^2); %I don't quite know why devide by dV.^2

% calculate a dV estimate at each pixel, based on the linear regression.
imgs2 = zeros(size(imgs)); 
corrmat = repmat(corrimg, [1 1 L]);
imgs2 = imgs./corrmat;  % This will give NaN where corrmat == 0. ... also, why do this? The imgs2 will be low for high correlation
clear corrmat

% Look at the residuals to get a noise at each pixel
sigmaimg = mean((imgs2 - dVmat).^2,3); 

weightimg = (1./sigmaimg); %The weighting is higher for pixels with better lower noise residuals
