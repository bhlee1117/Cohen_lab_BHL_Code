function [out, scaleImgs] = SeeResiduals_mpc(mov, refSig, remOffset); 
...
if refSig == 0;  % Then only subtract the average image from each frame.
    ...
    Avgmov = mean(mov,3);
    dmov = (mov - repmat(Avgmov, [1 1 nFrame]));
    dmovVec = tovec(dmov);
    C = dmovVec*I'*inv(I*I');  % Linear algebra to find the pseudo-inverse
