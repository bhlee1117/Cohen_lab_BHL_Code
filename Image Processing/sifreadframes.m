...
%Synopsis:
%
%  [data,back,ref]=sifread(file, startframe, nframes)
...
%
%        Modified by Alex Fields and Adam Cohen October 2008 to read a
%        specified number of frames.

function [data,back,ref]=sifread(file, startindex, period, ncycles)
f=fopen(file,'r');
if f < 0
...
skipLines(f,1);
[data,next]=readSection(f, startindex, period, ncycles);
if nargout > 1 & next == 1
...
%
function [info,next]=readSection(f, startindex, period, ncycles)
o=fscanf(f,'%d',6);
...
skipBytes(f,4*prod(s)*(startindex-1));
%skipBytes(f,9);
info.imageData = (zeros([s, period]));
info.timetrace = zeros(1,ncycles*period);
for j = 1:ncycles;
%     j
    temp = reshape(fread(f,prod(s)*period,'uint32'),[s period])/(2^10);
    info.imageData = info.imageData + temp;
    info.timetrace((period*(j-1) + 1):period*j) = squeeze(mean(mean(temp)));
end;
