function out = read_tifs2(filename, firstimage, lastimage)
...
img1 = imread([filename '_' num2str(firstimage) '.tif'], 'tif');     

[rows, cols] = size(img1);
out = uint16(zeros(rows, cols, lastimage - firstimage + 1));

for j = firstimage:lastimage
    out(:,:,j-firstimage + 1) = imread([filename '_' num2str(j) '.tif'], 'tif');     
end;
out = double(out);
out = out - 2^15;