...
b=bwlabel(Virmen_data(:,3)>pos);
pos_reach_time=NaN(size(laps,1),1);

for l=1:size(laps,1)
tmp=find(b(laps(l,1):laps(l,2))'==l);
tmp=tmp+laps(l,1)-1;

if isempty(tmp) %truncated end
pos_reach_time(l)=NaN;    
b=b+1;
else if abs(Virmen_data(tmp(1),3)-pos)>10 %late start
pos_reach_time(l)=NaN;    
else
