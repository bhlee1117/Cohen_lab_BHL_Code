...
clear
clc;
cd '/Volumes/ByungHun2TB/Prism_Optopatch_backup';
[~, ~, raw] = xlsread(['/Volumes/ByungHun2TB/Prism_OptopatchData_Arrangement.xlsx'], 'Sheet1', 'B5:P175');
...
figure(13); clf; tiledlayout(1,2);
nexttile([1 1]);
[binnedZ binX binY]=show3Dbinning(SpikeAmp, 8, 7, 'image'); hold all
allX=[]; allY=[]; allZ=[];
  for n = 1:numel(SpikeAmp)
        timeAxis = SpikeAmp{n}(1, 2:end);
        xAxis = SpikeAmp{n}(2:end, 1);
...
nexttile([1 1]);
[binnedZ binX binY]=show3Dbinning(SpikeAUC, 8, 7, 'image'); hold all
allX=[]; allY=[]; allZ=[];
  for n = 1:numel(SpikeAUC)
        timeAxis = SpikeAUC{n}(1, 2:end);
        xAxis = SpikeAUC{n}(2:end, 1);
