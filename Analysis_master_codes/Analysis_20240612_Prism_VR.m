...
%       Result.tracesPCA=[Result.tracesPCA -(tovec(mov_res)'*tovec(Result.pcaMask))'];
    ...
    Result.traces_bvMask=Result.traces_bvMask(:,1:frm_end);
...
tr_res=squeeze(SeeResiduals(permute(Result.traces,[1 3 2]),y_fit2));
    ...
    for n=1:size(Result.traces,1)
        ...
        if n==ref_trace(f)
            for t=1:length(tN)-1
                ...
                [sp_temp, pks, prom]=find_spike_bh(tr_tmp_norm,7,4);
...
for f=[20]
    ...
    show_traces_spikes(Result.normTraces,Result.spike,[Result.SpClass; double(Result.motionReject)]);
    %save(fullfile(fpath{f},'PC_Result.mat'),'Result','fpath','-v7.3')
end
