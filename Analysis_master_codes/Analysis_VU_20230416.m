...
disp([fpath{i}])
load([fpath{i} '/output_data.mat'])
Blue=Device_Data{1, 2}.buffered_tasks(1, 1).channels(1, 1).data;
a=find(Device_Data{1, 2}.Counter_Inputs(1, 1).data(2:end)-Device_Data{1, 2}.Counter_Inputs(1, 1).data(1:end-1));
frm_rate=(a(2)-a(1))*DAQ_rate;
sz=Device_Data{1, 3}.ROI([2 4]);
mov_mc=double(readBinMov([fpath{i} '/mc.bin'],sz(2),sz(1)));
t=[1:size(mov_mc,3)]*frm_rate; t(t>length(Blue)*DAQ_rate)=[];


[roi int]=clicky(mov_mc); int=int-median(int,1);
close all
figure; ax1=[]; scale=120;
tiledlayout(5,1)
nexttile([1 1])
imshow2(mean(mov_mc,3),[]); hold all
for r=1:length(roi); plot(roi{r}(:,1),roi{r}(:,2),'linewidth',2); hold all; end
ax1=[ax1 nexttile([3 1])]; 
plot(t,-int(1:length(t),:)+[1:length(roi)]*scale); set(gca,'FontSize',18)
title(fpath_mac2window([fpath{i}]))
%ylim([min(-int(10:length(t))) max(-int(10:length(t)))])
ylim([0 (length(roi)+1)*scale])
...
[centers radii]=Cell_segment_circle_2x(im_G,0.7);
Result{i}.centers=cell_detection_manual(mean(mov_mc,3),centers,[]);
Result{i}.c_ftprnt=mask_footprint(Result{i}.centers,movmean(mov_res(:,:,1000:end),10,3),[],17);
