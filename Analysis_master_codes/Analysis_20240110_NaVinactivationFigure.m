...
for N=1:length(Cell_list) %Cell number
    ...
    for d=1:4 % 4 states, Iso, Awake, Ket, CPP
        ...
        for j=1:length(Scratch_state_list) %find cells match to state
            ...
            Ad_Result_NList{N,d}.Waveform=sum(Ad_Result_NList{N,d}.Blue>0,2)>10000; %waveform==0 : pulse + ramp, waveform==1 : triangles
...
figure; t_Ramp=[1942+500:11941+500]; scale=0.15;
for g=1:length(noi)
    N=noi(g);
    tmp=find(Ad_Result_NList{N,d}.Waveform==0);
    F_zero=Ad_Result_NList{N,d}.ref_im(show_ind(g));
    show_tr=Ad_Result_NList{N,d}.trace(show_ind(g),:);
    show_tr_CS=show_tr; show_tr_CS(Ad_Result_NList{N,d}.CS_trace(show_ind(g),:)==0)=NaN;
    plot(show_tr(t_Ramp)/F_zero+scale*(g-1),'color',cTrace); hold all; axis off
    plot(show_tr_CS(t_Ramp)/F_zero+scale*(g-1),'color',cCS)
end
plot(Ad_Result_NList{N,d}.Blue(show_ind(g),t_Ramp)/30-scale,'color',cBlue); axis off
...
BinTime=100; BinTime_R=150;
plotInd={[1 2],[1],[1 2 3],[1],[1 2],[1],[1 3],[5 6],[1 2],[1 2],[1]};
for n=1:size(Ad_Result_NList,1) %Cell number
    ...
    for s=1:length(Pulse_bw) %stimulation
...
%% CS rate at Rheobase 
RheoBase_norm=[];
trianglewaveform_list=cellfun(@(x) find(x.Waveform==1), Ad_Result_NList(:,1), 'UniformOutput', false);
pulsewaveform_list=cellfun(@(x) find(x.Waveform==0), Ad_Result_NList(:,1), 'UniformOutput', false);
d=1; g=1; g2=1;
SpikeInfoMat=[]; SpikeInfoMatStep=[];
for N=1:size(Ad_Result_NList,1)

TriangleTrials=trianglewaveform_list{N};
PulseTrials=pulsewaveform_list{N};

FstSpikeBlueAOTF=[];
for trial=TriangleTrials'
Sptime=find(Ad_Result_NList{N,d}.spike(trial,:).*double(Ad_Result_NList{N,d}.Blue(trial,:)>0));
FstSpikeBlueAOTF=[FstSpikeBlueAOTF Ad_Result_NList{N,d}.Blue(trial,Sptime(1:3))];
end
RheoBase_norm(N)=mean(FstSpikeBlueAOTF);
RheoBase=Ad_Result_NList{N,d}.Blue/RheoBase_norm(N);

t_ramp=NaN(size(Ad_Result_NList{N,d}.trace));
t_ramp_start=NaN(size(Ad_Result_NList{N,d}.trace,1),1);

for trial=PulseTrials'
    bwBlue=bwlabel(Ad_Result_NList{N,d}.Blue(trial,:));
    t_ramp(trial,bwBlue==max(bwBlue))=1;
    t_ramp_start(trial)=find(bwBlue==max(bwBlue),1);
end

simplespikeMat=t_ramp.*Ad_Result_NList{N,d}.spike.*(1-Ad_Result_NList{N,d}.CS_trace);
complexspikeMat=t_ramp.*Ad_Result_NList{N,d}.spike.*(Ad_Result_NList{N,d}.CS_trace);

[rep sptime]=find(simplespikeMat==1);
[repCS sptimeCS]=find(complexspikeMat==1);

for trial=PulseTrials'
SpikeInfoMat{g,1}=[[sptime(rep==trial)-t_ramp_start(trial) RheoBase(trial,sptime(rep==trial))' ones(sum(rep==trial),1)];...
                   [sptimeCS(repCS==trial)-t_ramp_start(trial) RheoBase(trial,sptimeCS(repCS==trial))' ones(sum(repCS==trial),1)*2]];
g=g+1;
end

t_step=NaN(size(Ad_Result_NList{N,d}.trace));
t_step_start=NaN(size(Ad_Result_NList{N,d}.trace,1),5);

for trial=PulseTrials'
    bwBlue=bwlabel(Ad_Result_NList{N,d}.Blue(trial,:));
    t_step(trial,ismember(bwBlue,[max(bwBlue)+[-5:-1]]))=1;
    for s=1:5 %stimulation bw
    t_step_start(trial,s)=find(ismember(bwBlue,[max(bwBlue)-s]),1);
    end
end

simplespikePulseMat=t_step.*Ad_Result_NList{N,d}.spike.*(1-Ad_Result_NList{N,d}.CS_trace);
complexspikePulseMat=t_step.*Ad_Result_NList{N,d}.spike.*(Ad_Result_NList{N,d}.CS_trace);

[rep sptime]=find(simplespikePulseMat==1);
[repCS sptimeCS]=find(complexspikePulseMat==1);

for trial=PulseTrials'
SptimeOnset=mod(sptime(rep==trial)-min(t_step_start(trial,:)),t_step_start(trial,1)-t_step_start(trial,2));
SptimeOnsetCS=mod(sptimeCS(repCS==trial)-min(t_step_start(trial,:)),t_step_start(trial,1)-t_step_start(trial,2));

SpikeInfoMatStep{g2,1}=[[SptimeOnset RheoBase(trial,sptime(rep==trial))' ones(sum(rep==trial),1)];...
                   [SptimeOnsetCS RheoBase(trial,sptimeCS(repCS==trial))' ones(sum(repCS==trial),1)*2]];
g2=g2+1;
end
end
    
figure(10); clf; tiledlayout(3,1);
RheoEdge=[0.5:0.4:6.6];
BinCountTotal = cellfun(@(x) histcounts(x(:,2),RheoEdge),SpikeInfoMat,'UniformOutput',false);
BinCountCS = cellfun(@(x) histcounts(x(x(:,3)==2,2),RheoEdge),SpikeInfoMat,'UniformOutput',false);
FractionCS = cellfun(@(x,y) x./y,BinCountCS,BinCountTotal,'UniformOutput',false);

nexttile([2 1])
TotalSp=sum(cell2mat(BinCountTotal));
TotalCS=sum(cell2mat(BinCountCS));
EdgeCenter=(RheoEdge(2:end)+RheoEdge(1:end-1))/2;
bar(EdgeCenter,TotalSp,'FaceColor',[0.6 0.6 0.6]); hold all
bar(EdgeCenter,TotalCS,'FaceColor',[1 0 0],'FaceAlpha',0.7); hold all
xlabel('Optical Rheobase');
ylabel('Spike Counts')
legend({'Total spikes','Complex spikes'})

nexttile([1 1])
ShowDat=cell2mat(FractionCS);
errorbar(EdgeCenter,mean(ShowDat,1,'omitnan'),std(ShowDat,0,1,'omitnan')/sqrt(size(ShowDat,1)),'r')
xlabel('Optical Rheobase');
ylabel('Fraction of complex spike')


figure(11); clf; tiledlayout(3,1);
timeEdge=[0:40:520]; Strength_bin=[0 1.5 7];

BinCountTotal = cellfun(@(x) histcounts(x(:,1),timeEdge),SpikeInfoMatStep,'UniformOutput',false);
BinCountCS = cellfun(@(x) histcounts(x(x(:,3)==2,1),timeEdge),SpikeInfoMatStep,'UniformOutput',false);
FractionCS = cellfun(@(x,y) x./y,BinCountCS,BinCountTotal,'UniformOutput',false);

nexttile([2 1])
TotalSp=sum(cell2mat(BinCountTotal));
TotalCS=sum(cell2mat(BinCountCS));
TCenter=(timeEdge(2:end)+timeEdge(1:end-1))/2;
bar(TCenter,TotalSp,'FaceColor',[0.6 0.6 0.6]); hold all
bar(TCenter,TotalCS,'FaceColor',[1 0 0],'FaceAlpha',0.7); hold all
xlabel('Time after stimulation onset');
ylabel('Spike Counts')
legend({'Total spikes','Complex spikes'})

nexttile([1 1])
cmap=[1 0 0; 0.5 0 1];
for s=1:length(Strength_bin)-1

BinCountTotal = cellfun(@(x) histcounts(x(x(:,2)>Strength_bin(s) & x(:,2)<Strength_bin(s+1),1),timeEdge),SpikeInfoMatStep,'UniformOutput',false);
BinCountCS = cellfun(@(x) histcounts(x(x(:,2)>Strength_bin(s) & x(:,2)<Strength_bin(s+1) & x(:,3)==2,1),timeEdge),SpikeInfoMatStep,'UniformOutput',false);
FractionCS = cellfun(@(x,y) x./y,BinCountCS,BinCountTotal,'UniformOutput',false);

ShowDat=cell2mat(FractionCS);
errorbar(TCenter,mean(ShowDat,1,'omitnan'),std(ShowDat,0,1,'omitnan')/sqrt(size(ShowDat,1)),'color',cmap(s,:)); hold all
xlabel('Time after stimulation onset');
ylabel('Fraction of complex spike')
end
