...
%%
TrRaw=Result.normTraces;
FtprintSum=squeeze(sum(Result.ftprnt,[1 2]));
%TrRaw=TrRaw./FtprintSum;
[nROI nTime]=size(TrRaw);
sz=size(Result.ref_im);
TrRaw=TrRaw(Result.dist_order,:);
TrSub=get_subthreshold(TrRaw,Result.spike(1,:),7,17);
figure(23); clf;
l=plot(TrRaw');
arrayfun(@(l,c) set(l,'Color',c{:}),l,num2cell(gen_colormap(Plasma,nROI),2));

sp=Result.SpClass(1,:);
[~, STmat]=get_STA(TrRaw,sp,30,20);
[~, STmat_sp]=get_STA(sp,sp,30,20);
ind2avg=squeeze(sum(STmat_sp,3))==1;
STAs=squeeze(mean(STmat(:,ind2avg,:),2));
STAs=STAs-median(STAs(:,1:10),2);

F0PCA=get_F0PCA(TrSub,2)';
F0=tovec(Result.ref_im-100)'*tovec(Result.ftprnt(:,:,Result.dist_order));

ind2avgMov=find(ind2avg); ind2avgMov=ind2avgMov(1:70);
mov_STA=[];
for j=1:3
    mov_STA=cat(3,mov_STA,readBinMov(fullfile(fpath{f},['STA_MAT_SS_' num2str(j) '.bin']),sz(1),sz(2)));
end
STAmovie=-mean(reshape(mov_STA(:,:,:,ind2avgMov),sz(1),sz(2),51,[]),4,'omitnan');
STAmovie=-(STAmovie-median(STAmovie(:,:,1:15),3));
%STAs2=tovec(-STAmovie)'*tovec(Result.ftprnt(:,:,Result.dist_order));
F0_robust=get_robustdFF(STAmovie(:,:,31+[-3:5]),Result.ftprnt(:,:,Result.dist_order),Result.ref_im-100);

F0_ref=mean(STAs(:,30+[10:19]),2)';
%% Plot and compare different methods
omitROI=[6 7 20];
figure(6); clf; cax=[-0.1 1];
show_ROI=setdiff([1:nROI],omitROI);
dax=Result.interDendDist(1,Result.dist_order);
dax=dax(show_ROI);

normSTAF0=STAs./F0'; normSTAF0=normSTAF0./normSTAF0(14,31);
normSTAF0_rbst=STAs./F0_robust'; normSTAF0_rbst=normSTAF0_rbst./normSTAF0_rbst(14,31);
normSTAF0_ref=STAs./F0_ref'; normSTAF0_ref=normSTAF0_ref./normSTAF0_ref(14,31);
normSTAF0_PCA=STAs./F0PCA'; normSTAF0_PCA=normSTAF0_PCA./normSTAF0_PCA(14,31);

nexttile([1 1]);
imagesc(normSTAF0(show_ROI,:),cax); title('F0'); set_kymoYtick(dax);
nexttile([1 1]);
imagesc(normSTAF0_rbst(show_ROI,:),cax); title(['Robust \DeltaF/F,' newline '(Wong-Campos et al., 2023)']); set_kymoYtick(dax);
nexttile([1 1]);
imagesc(normSTAF0_ref(show_ROI,:),cax); title(['F_{ref},' newline '(Park et al., 2025)']); set_kymoYtick(dax);
nexttile([1 1]);
imagesc(normSTAF0_PCA(show_ROI,:),cax); title('F_{std}'); set_kymoYtick(dax);
colorbar;

nexttile([1 1]);
scatter(dax,max(normSTAF0(show_ROI,30+[-2:4]),[],2),'k','filled');
ylim([0.3 1.5]); xlabel('Distance (\mum)'); ylabel('Normalized amplitude');
nexttile([1 1]);
scatter(dax,max(normSTAF0_rbst(show_ROI,30+[-2:4]),[],2),'k','filled');
ylim([0.3 1.5]); xlabel('Distance (\mum)'); ylabel('Normalized amplitude');
nexttile([1 1]);
scatter(dax,max(normSTAF0_ref(show_ROI,30+[-2:4]),[],2),'k','filled');
ylim([0.3 1.5]); xlabel('Distance (\mum)'); ylabel('Normalized amplitude');
nexttile([1 1]);
scatter(dax,max(normSTAF0_PCA(show_ROI,30+[-2:4]),[],2),'k','filled');
ylim([0.3 1.5]); xlabel('Distance (\mum)'); ylabel('Normalized amplitude');
set_fontsize(20);
