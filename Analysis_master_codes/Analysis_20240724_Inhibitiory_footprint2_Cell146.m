...
colormap('gray')
%%
BA_Rois={[4 8 7 10 9 6],[15 13]}; 

spike_time=tovec(find(Result.spike(2,:))'+[-5:50]);
...
inhibit_time=unique([find(ismember(bwlabel(transApical{2,2}.interval),find(transApical{2,2}.amp>Amplitude_threshold))) find(ismember(bwlabel(transBasal{2,2}.interval),find(transBasal{2,2}.amp>Amplitude_threshold)))]);
excite_time=unique([find(ismember(bwlabel(transApical{1,1}.interval),find(transApical{1,1}.amp>Amplitude_threshold))) find(ismember(bwlabel(transBasal{1,1}.interval),find(transBasal{1,1}.amp>Amplitude_threshold)))]);
...
subMov=tovec(imgaussfilt3(subMov,[0.5 0.5 0.1]));
subMov2=tovec(imgaussfilt3(subMov2,[0.5 0.5 0.1]));
...
nPCs=12;
eigImg=toimg(tovec(mov_blueOff_exc)*V(:,1:nPCs),size(mov_res,1),size(mov_res,2));
figure(9); clf; 
for n=1:nPCs
    nexttile([1 1])
    imshow2(imgaussfilt(eigImg(5:end-5,5:end-5,n),2),[])
