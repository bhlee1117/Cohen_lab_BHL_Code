...
for t=1:length(t_slide)-1
    ...
    [corrMat_moving(t,:,1), t_lag]=xcorr(SilentTrace_BA(1,t_corr),SilentTrace_BA(2,t_corr),window_sz,'normalized');
...
Corr_trace=mean(corrAmp_moving_int(window_sz+[-10:10],:),1,'omitnan');
...
nexttile([1 2]);
dendrogram(Z_ref,0,'ColorThreshold',570); axis off
...
binEdge=[-5:0.1:5]; up_thres=1.7; dw_thres=1.5; blueTargetF=mean(reshape(NormalizedTrace(ref_ROI{f}(1),bAP_ref'+[6:10]),length(bAP_ref),[]),[1 2]);
