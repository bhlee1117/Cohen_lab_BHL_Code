...
for i=1%:length(grps)
    ...
    for g=2:length(grps{i})
        dist=distance_mat(Result{grps{i}(1)}.coord,Result{goi}.coord);
        [d arg]=min((dist),[],2);
...
%     show_footprnt(Result{goi}.c_ftprnt,Result{goi}.ref_im);
    ...
    % end
% 
 Lap_FR=get_LapFR_Virmen(traces_sp,place_bin,Virmen_align,0.007,[1:size(traces_sp)]);
...
moviesc(Lap_FR)
%%
show_traces_align_Position_Virmen(traces_F,traces_sp,115*0.3,[-500:10000],Virmen_align,6)
