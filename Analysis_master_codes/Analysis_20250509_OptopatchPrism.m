...
for i=unqInd(8)'
    ...
    for j=SameCellInd'
        ...
        if size(Device_Data{1, 6}.pattern_stack,3)>1
            [Result.blueDMDimg Result.bluePatt Result.blueDMDcoord]=get_blueDMDPatt(Device_Data,'stack');
        else
            [Result.blueDMDimg Result.bluePatt Result.blueDMDcoord]=get_blueDMDPatt(Device_Data,'normal');
        end
        Result.ROIpoly=ROIpoly;
        sz=double(Device_Data{1, 3}.ROI([2 4]));
        ...
        [~, regTform]=imReg_faster(avgImg,mean(mov_mc,3));
        Rfixed = imref2d(size(mean(mov_mc,3)));
        
        ROIpoly_shift=cellfun(@(x) transformPointsForward(regTform,x),Result.ROIpoly,'UniformOutput',false);

        Result.ROIpoly=ROIpoly_shift;
        Rfixed = imref2d(sz([2 1]));

        [x, y] = meshgrid(1:sz(1), 1:sz(2));
        Result.bvMask=[];
        for n=1:size(bvMask,3)
            %Result.bvMask(:,:,n) = interp2(x, y, bvMask(:,:,n), x+offsetX, y+offsetY, 'linear', 0);
            Result.bvMask(:,:,n) = imwarp(bvMask(:,:,n), regTform, 'OutputView', Rfixed);
        end
...
for i=[152]
    ...
    for j=1:totalNbin
        ...
        mov_mc=double(readBinMov_times([fpath{i} '/mc_ShutterReg' num2str(j,'%02d') '.bin'],sz(2),sz(1),[1:readFrame(j)]));
        ...
        Result.dirtTraceSplit=[Result.dirtTraceSplit, (tovec(dirtMov_dilate)'*tovec(Result.ftprntSplit))'];        
    end
...
for i=unqInd([8])'
    ...
    for j=SameCellInd'
        ...
        Result.Structure=imwarp(RegImg, regTform, 'OutputView', Rfixed);
        Result.Structure_bin=imwarp(RegImg, regTform, 'OutputView', Rfixed);
        nexttile([1 1])
        imshow2(imfuse(Result.ref_im,Result.Structure),[]);
        save([fpath{j} '/OP_Result.mat'],'Result','-v7.3')
        disp(['OP_Result.mat is save to ' fpath{j}])
    end
end
