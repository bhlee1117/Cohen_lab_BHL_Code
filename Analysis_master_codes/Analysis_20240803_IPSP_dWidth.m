...
spike= find_spike_bh(tr_norm,5,3);
F0=tovec(Result.ref_im)'*tovec(Result.ftprnt.*double(max(Result.bvMask,[],3)==0));
NormTrace=Result.traces_bvMask./F0';
NormTrace=NormTrace-prctile(NormTrace,30,2);
subth_trace=get_subthreshold(NormTrace,spike(ref_spike_ROI,:),5,15);
spike_time=tovec(find(spike(ref_spike_ROI,:))'+[-5:50]);
...
plot(NormTrace(ref_spike_ROI,:)); hold all
plot(subth_trace(ref_spike_ROI,:))
plot([1 size(NormTrace,2)],[0.005 0.005],'r')
...
%%
figure(21); clf; tiledlayout(6,2)
binEdges=[-0.05:0.001:0.05];
nexttile([2 2])
histogram(NormTrace(ref_spike_ROI,BlueTime{1} & spike_erode_trace ==0),binEdges,'Normalization','probability'); hold all
histogram(NormTrace(ref_spike_ROI,BlueTime{2} & spike_erode_trace ==0),binEdges,'Normalization','probability');
xlabel('\DeltaF/F')
ylabel('Probability')
...
nexttile([1 1])
histogram(NormTrace(ref_spike_ROI,BlueTime{1} & spike_erode_trace ==0),binEdges,'Normalization','probability'); hold all
histogram(NormTrace(ref_spike_ROI,bwBlue==b & spike_erode_trace ==0),binEdges,'Normalization','probability'); 
title(['During ' num2str(sum(bwBlue==b)/1000) 's stim., skewness ' num2str(skewness(NormTrace(ref_spike_ROI,bwBlue==b & spike_erode_trace ==0)),2)])
legend({'Blue off','Blue on'},'Location','west')
xlabel('\DeltaF/F')
ylabel('Probability')
end
%% pca analysis
...
%%
putative_inh_time=[2350]; nTau=[1:320];
Rfixed = imref2d([size(Result.ref_im,1) size(Result.ref_im,2)]);
inverseTform = invert(Result.tform);
revertedStruct = Result.Structure;
revertedStruct_bin = Result.Structure_bin;
revertedStruct_bin=revertedStruct_bin>0.05;
revertedStruct_bin=revertedStruct_bin(bound:end-bound,bound:end-bound);
revertedStruct=revertedStruct(bound:end-bound,bound:end-bound);
for j=1:length(putative_inh_time)
mov_func=imgaussfilt3(-mov_res(bound:end-bound,bound:end-bound,putative_inh_time(j)+nTau).*revertedStruct_bin,[5 5 0.1])./imgaussfilt(Result.ref_im(bound:end-bound,bound:end-bound),3);
  colorMov=grs2rgb(tovec(mov_func),jet(300),-0.02,0.03);
  colorMov=reshape(colorMov,size(mov_res,1)-2*bound+1,size(mov_res,2)-2*bound+1,length(nTau),[]);
  colorMov=permute(colorMov,[1 2 4 3]);
 mov_show=colorMov.*revertedStruct*0.7;
 %mov_show=mov_show(bound:end-bound,bound:end-bound,:,:);
 writeMov4d([fpath{146} '/PutativeInhibitMov_' num2str(j)],mov_show,nTau,15,1,[])
end
%%
[~, BlueOffTime]=get_blueoffTrace(zeros(1,length(Result.Blue)),Result.Blue,100);
BlueTime= {BlueOffTime,Result.Blue>0}; % Blue off: b=1; Blue on: b=2; Excitation: inh=1; inhibition: inh=2;
