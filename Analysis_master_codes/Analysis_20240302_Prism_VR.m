...
for f=4
    ...
    tr=Result.normTraces(:,1:180200);
    tr_ref=Result.normTraces([1 2 3],1:180200);

    %nROI=size(PC_Result{i}.normTraces,1);
    nROI=size(tr,1);
    DOI{f}=setdiff([2:nROI],[1 2 3]);
    %sp=PC_Result{i}.spike;
    sp=find_spike_bh(tr-movmedian(tr,50,2),5,3);
    sp_ref=find_spike_bh(tr_ref-movmedian(tr_ref,100,2),5,3);
    [wvletTr wvletF] = cwt(Result.im_corr(1:180200),1000); % Compute the CWT
    ...
    sp(:,motionArtTrace>4)=0; sp_ref(:,motionArtTrace>4)=0;
    sp_ref=max(sp_ref,[],1);

    [~, shift]=max(reshape(tr(1,find(sp_ref)+[-1:0]'),2,[]),[],1);
    shift=shift-2;
    sp_time_Soma = find(sp_ref>0)+shift;
    sp_soma=zeros(1,size(tr,2));
    sp_soma(sp_time_Soma)=1;

    tr_sub=tr_ref(1,:)-movprc(tr_ref(1,:),300,20);
...
sp_total=max([sp_soma; sp(DOI{i},:)],[],1);
bAP_ind=zeros(1,size(tr,2));
bAP_ind(unique(find(sp_soma)'+[0:2]))=1;

SpikeClassMat=zeros(3,size(tr,2));
SpikeClassMat(1,:)=sp_total.*bAP_ind; %bAPs
SpikeClassMat(2,:)=sp_total.*CS_trace; %Complex spikes
...
Result.CStrace=CS_trace;
    ...
    fileInd=ceil(find(PC_Result{f}.SpikeClassMat)/time_segment);
    frameInd=mod(find(PC_Result{f}.SpikeClassMat),time_segment);
...
PC_Result{g}.CS_time(CS)= find(CS_trace_spike==CS,1,'first');
    ...
    %DOI{i}=setdiff([2:nROI],ref_ROI{i});
    
    
    

    DOI{i}=setdiff([2:nROI],[1 2 3]);
