...
nTau={[-300:20],[-300:200],[-20:20]}; %SS, CS, dSP
nTau_bAP=[-20:20];
...
for c=1:size(STA_CSmat_sub,2)
    ...
    [fitresult, gof] = fit([t:t+1]'*dt, squeeze(STA_CSmat(n,c,[t:t+1]*dt)),ft);
...
for c=1:size(STA_SSmat,2)
    ...
    for t=1:-nTau{1}(1)/dt
        for n=1:size(STA_SSmat,1)
    [fitresult, gof] = fit([t:t+1]'*dt, squeeze(STA_SSmat(n,c,[t:t+1]*dt)),ft);
...
figure(13); clf;
nexttile([1 1])
plot(sum(SSprop.preArea,1),SSprop.dVdx,'.'); hold all
plot(sum(CSprop.preArea,1),CSprop.dVdx,'.'); hold all
...
nexttile([1 1])
h=histogram(squeeze(mean(SSprop.dVdt(rois{1},:,end),1)),30,'Normalization','probability'); hold all
histogram(squeeze(mean(CSprop.dVdt(rois{1},:,end),1)),h.BinEdges,'Normalization','probability')
legend({'SS','CS'});
xlabel('dV/dt (Basal)')

nexttile([1 1])
h=histogram(squeeze(mean(SSprop.dVdt(rois{2},:,end),1)),30,'Normalization','probability'); hold all
histogram(squeeze(mean(CSprop.dVdt(rois{2},:,end),1)),h.BinEdges,'Normalization','probability')
legend({'SS','CS'});
xlabel('dV/dt (Apical)')
...
nexttile([1 1])
h=histogram(sum(SSprop.preArea(rois{1},:),1),30,'Normalization','probability'); hold all
histogram(sum(CSprop.preArea(rois{1},:),1),h.BinEdges,'Normalization','probability')
legend({'SS','CS'});
xlabel('Sum of Pre-area (Basal)')

nexttile([1 1])
h=histogram(sum(SSprop.preArea(rois{2},:),1),30,'Normalization','probability'); hold all
histogram(sum(CSprop.preArea(rois{2},:),1),h.BinEdges,'Normalization','probability')
legend({'SS','CS'});
xlabel('Sum of Pre-area (Apical)')
...
figure(11); clf;
title_str={'Apical D1','Apical D2','Basal D1','Basal D2','Basal D3'};
for t=-5:1:-1
     g=1;
for n=[5 6 13 14 15]
    nexttile([1 1])
...
figure(6); clf;
nexttile([1 1])
imagesc(squeeze(mean(STA_CSmat(dist_order,find(CSprop.dVdx>0.005),:),2)),[-0.5 3])
colormap(turbo)
nexttile([1 1])
imagesc(squeeze(mean(STA_CSmat(dist_order,find(CSprop.dVdx<-0.005),:),2)),[-0.5 3])
colormap(turbo)
nexttile([1 1])
l=plot(squeeze(mean(STA_CSmat(dist_order,find(CSprop.dVdx>0.005),:),2))');
arrayfun(@(l,c) set(l,'Color',c{:}),l,num2cell(turbo(nROI),2))
ylim([-2 10])
nexttile([1 1])
l=plot(squeeze(mean(STA_CSmat(dist_order,find(CSprop.dVdx<-0.005),:),2))');
...
ISImat(ISImat==0)=NaN;
nexttile([1 1])
plot([1:size(ISImat,2)],ISImat,'color',[0.6 0.6 0.6]); hold all
errorbar([1:size(ISImat,2)],mean(ISImat,1,'omitnan'),std(ISImat,0,1,'omitnan'),'color',[1 0 0]); hold all
nexttile([1 1])
plot(mean(ISImat(:,1),2),mean(CSprop.area(rois{2},:),1),'.'); hold all
plot(mean(ISImat(:,2),2),mean(CSprop.area(rois{2},:),1),'.')

%%
figure(7); clf;
ax1=nexttile([1 1]);
l=plot(squeeze(mean(STA_CSmat(dist_order,find(CSprop.intercept_preArea<prctile(CSprop.intercept_preArea,30)),:),2))');
arrayfun(@(l,c) set(l,'Color',c{:}),l,num2cell(turbo(nROI),2))
ax2=nexttile([1 1]);
l=plot(squeeze(mean(STA_CSmat(dist_order,find(CSprop.intercept_preArea>prctile(CSprop.intercept_preArea,70)),:),2))');
arrayfun(@(l,c) set(l,'Color',c{:}),l,num2cell(turbo(nROI),2))
linkaxes([ax1 ax2],'xy')
...
nexttile([1 1])
imagesc(CSprop.preArea(dist_order,arg)')
title(['Pre spike Area'])
nexttile([1 1])
imagesc(CSprop.area(dist_order,arg)')
title(['Complex spike Area'])
nexttile([1 1])
imagesc(CSprop.ADPamp(dist_order,arg)')
...
for c=c_show
    ...
    colormap("turbo")
    title([num2str(c) ' , ' num2str(sum(CSprop.area(:,c),1),2),' , ' num2str(sum(CSprop.ADPamp(rois{2},c)),2)])
end
linkaxes(ax1,'xy')
%%
SkelDend = Skeletonize_dendrite(Result.ref_im,4,0.02,25);
