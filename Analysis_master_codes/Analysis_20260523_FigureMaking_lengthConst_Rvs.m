...
for f=f_local
    ...
    for b=1:size(Result.blueDMDimg,3)
        ...
        if length(frame2read)>1000
            ...
            ftprintscoord=get_coord(Result.ftprnt);
            SomaApicalcoord=ftprintscoord(ismember(Result.roilabel,[2 3 5]),:);
            [~, principal_axis]= projectTrunkaxis(SomaApicalcoord);
            LocalStimResult{g}.dendaxis1d = (ftprintscoord - ftprintscoord(1,:))* principal_axis;
            LocalStimResult{g}.principal_axis=principal_axis;
...
for Mov2See=[1:length(LocalStimResult)]
    ...
    STAtr=movmean(LocalStimResult{Mov2See}.STAtr,5,2,'omitnan');
    if any(~isnan(STAtr(:)))
        ss=squeeze(mean(STAtr,[1],'omitnan'));
        ss(-nTau(1)+[-1:50])=NaN;
        ss=interpolateNaN(movmean(ss,15,'omitnan'));
        ...
        STAtr=permute(SeeResiduals(permute(STAtr,[1 3 2]),ss),[1 3 2]);
        STAtr=STAtr-median(STAtr(:,1:-nTau(1)),2,'omitnan');
        ...
        for t=1:length(t2show)
            ...
            if ~isempty(LocalStimResult{Mov2See}.triggerPoint)
                STAtr_mean=mean(STAtr(:,-nTau(1)+frame2avg),2,'omitnan');
...
% for p=path2show'
%     nexttile([1 1])
%     scatter(Lstaall.dendaxis{p},Lstaall.VoltageSTA{p},19,'filled'); hold all;
%     plot(Lstaall.VoltageFit{p},'r');
%     title(sprintf('StimInd: %2.0f, Lc: %3.0f, R^2: %0.2f',Lstaall.StimInd{p},Lstaall.Fit_length{p},Lstaall.Fit_R2{p}));
% end
% 
% figure(89); clf;
% for p=path2show'
%     nexttile([1 1])
%     Ftcoord=get_coord(LocalStimResult{Lstaall.StimInd{p}}.ftprnt);
%     imshow2(LocalStimResult{Lstaall.StimInd{p}}.ref_im,[]); hold all;
%     scatter(Ftcoord(LocalStimResult{Lstaall.StimInd{p}}.stimInd,1),Ftcoord(LocalStimResult{Lstaall.StimInd{p}}.stimInd,2),40,[0 0.5 1],'filled');
%     title(sprintf('Lc: %3.0f, R^2: %0.2f',Lstaall.Fit_length{p},Lstaall.Fit_R2{p}));
% end

figure(90); clf;
nexttile([1 1]);
scatter(cell2mat(Lstaall.StimLocation1d(path2show)),cell2mat(Lstaall.Fit_length(path2show)),20,vec2cmap(cell2mat(Lstaall.Fit_R2(path2show)),'turbo'),'filled')
xlabel('Stimation laminar location (µm)'); ylabel('Decay length (µm)');
