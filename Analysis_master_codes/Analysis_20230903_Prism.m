cd '/Volumes/cohen_lab/Lab/Labmembers/Byung Hun Lee/Data/20230831_PP72_Prism'
clear
fpath_soma='/Volumes/cohen_lab/Lab/Labmembers/Byung Hun Lee/Data/20230831_PP72_Prism/190637Prism_PP72_mouse2_488_soma_step_30ms';
fpath_dend='/Volumes/cohen_lab/Lab/Labmembers/Byung Hun Lee/Data/20230831_PP72_Prism/191940Prism_PP72_mouse2_488_dend_step_30ms';
fpath={fpath_soma,fpath_dend};
%% Motion correction
...
StimTA_mov=zeros(size(ReconMov,1),size(ReconMov,2),length(window));
for i=1:length(triggerFrame)
StimTA_mov=StimTA_mov+ReconMov(:,:,triggerFrame(i)+window);    
end
StimTA_mov=StimTA_mov./length(triggerFrame);        
StimTA_mov=imgaussfilt3(StimTA_mov,[0.5 0.5 1]);
writeMov_wTrace([fpath{1} ,'_StimTA'],StimTA_mov,[],10,1,[-0.05 0.7],[],Blue_crop(triggerFrame(1)+window))
...
writeMov_wTrace([fpath{1} ,'_SpikeTA_2'],interpSpikeTA_mov,[],50,10,[-0.05 2],[],interSTA)
...
StimTA_mov=zeros(size(ReconMov,1),size(ReconMov,2),length(window));
for i=1:length(triggerFrame)
StimTA_mov=StimTA_mov+mov_res(:,:,triggerFrame(i)+window);    
end
StimTA_mov=StimTA_mov./length(triggerFrame);        
StimTA_mov=imgaussfilt3(StimTA_mov,[0.5 0.5 1]);
writeMov_wTrace([fpath{2} ,'_StimTA_dendriteStim'],StimTA_mov,[],10,1,[-0.05 2],[],Blue_crop(triggerFrame(1)+window))
...
interpSpikeTACS_mov=imgaussfilt3(interpSpikeTACS_mov,[1.5 1.5 3]);
interSTACS=interp1([1:length(STACS)],STACS,[1:originalSize(3) * interpolationFactor]/interpolationFactor);
figure;
writeMov_wTrace([fpath{2} ,'_SpikeTA_dendriteStimCS'],interpSpikeTACS_mov,[],50,5,[-0.05 2.7],[],interSTACS)

interpSpikeTASS_mov = interp3(SpikeTA_movSS, Xq, Yq, Zq, 'linear');
interpSpikeTASS_mov=imgaussfilt3(interpSpikeTASS_mov,[1.5 1.5 3]);
interSTASS=interp1([1:length(STASS)],STASS,[1:originalSize(3) * interpolationFactor]/interpolationFactor);
figure;
writeMov_wTrace([fpath{2} ,'_SpikeTA_dendriteStimSS'],interpSpikeTASS_mov,[],50,5,[-0.05 2.7],[],interSTASS)
