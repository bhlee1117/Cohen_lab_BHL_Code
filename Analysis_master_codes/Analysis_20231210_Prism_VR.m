...
sz=double(Device_Data{1, 3}.ROI([2 4]));
ref_time=[2000:3000];
mov_test=double(readBinMov_times([fpath{f} '/mc_ShutterReg' num2str(30,'%02d') '.bin'],sz(2),sz(1),ref_time));
...
load(fullfile(fpath{f},'mcTrace49.mat'));
frm_end=max(Device_Data{1, 2}.Counter_Inputs(1, 1).data);
...
mov_mc=double(readBinMov([fpath{f} '/mc_ShutterReg' num2str(49,'%02d') '.bin'],sz(2),sz(1)));
...
for i=1:length(PC_Result)
   ...
   nexttile([1 1])
   imagesc(rescale2(squeeze(mean(PC_Result{i}.STA,2)),2)); hold all
    colormap('turbo')
   l= plot(-rescale2(squeeze(mean(PC_Result{i}.STA,2)),2)'+[1:nROI]+0.5,'LineWidth',2);
