...
[VirFile]=uigetfile_n_dir();
       ...
       VirFile=fullfile(fpath{i},fileList.name);
    end
...
mov=double(readBinMov_times([fpath{i} '/frames1.bin'],sz(2),sz(1),[1:10]));
ref_im{i}=mean(mov,3);
...
isCellInBlue=diag(blueDMDimg(round(CellCoordinate(:,2)),round(CellCoordinate(:,1))));
[~, arg]=sort(isCellInBlue,'descend');
coord{i}=CellCoordinate(arg,:);
...
ROI{i}=round([coord{i}(:,1)-ROI_sz coord{i}(:,1)+ROI_sz ...
    coord{i}(:,2)-ROI_sz coord{i}(:,2)+ROI_sz]);
...
for n=1:size(coord{i},1)   
tmp=[];     
for t=1:size(t_seg,1)
%mov_seg=double(readBinMov_times_ROI([fpath '/frames1.bin'],sz(1),sz(2),[1:10],ROI));     
mov_seg=double(readBinMov_times_ROI([fpath{i} '/frames1.bin'],sz(1),sz(2),[t_seg(t,1):t_seg(t,2)],ROI{i}(n,:)));    
...
for n=1:size(coord{i},1)
mov_seg=double(readBinMov_times_ROI([fpath{i} '/frames1.bin'],sz(1),sz(2),[1:length(CamTrig)],ROI{i}));
...
figure(i); clf;
if size(coord{i},1) >5
tiledlayout(ceil((size(coord{i},1)+1)/2),6)
else
tiledlayout(size(coord{i},1)+1,3)
end
ax1=[];
ax2=[];

for noi=1:size(coord{i},1)
...
text(coord{i}(:,1)+2,coord{i}(:,2),num2str([1:size(coord{i},1)]'),'color','r','FontSize',10)
...
saveas(gca,[fullfile(fpath{i}, 'Brief.fig')])
