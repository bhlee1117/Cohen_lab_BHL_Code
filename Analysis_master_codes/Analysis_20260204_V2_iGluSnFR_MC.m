...
%% Read Voltage movie & motion correction
...
for f=[8]
    ...
    % laser artifacts
    PD_tr = Device_Data{1, 2}.buffered_tasks(1, 3).channels.data; % load diode
    v_cam_vsyn_diff = diff(cam1_vsyn)'; % down sample the PD trace
    ...
    for vs=2:length(VmovTimesegments)-1
        frm2read=[VmovTimesegments(vs):VmovTimesegments(vs+1)-1];
        [mov1, ~] = readBinMov_times(Vdata_filename, nRow1, nCol1,frm2read);
        mov1 = mov1 - 100;             % baseline

        period = 12;                              % artifact repeats every 12 frames
        ground_truth   = mean(mov1,3);            % mean image over time
        ...
        artifact_pattern = artifact_image ./ ground_truth;   % per-pattern artifact
        ...
        binned_PD_tr_norm_segment=binned_PD_tr_norm(frm2read-frm2read(1)+1);
        ...
        corrected_img=vm(corrected_img);
        [mov_mc,xyField]=optical_flow_motion_correction_LBH(corrected_img,mov_ref,'normcorre');
        ave_im=mean(mov_mc,3);
        mov_mc=vm(mov_mc);
        mov_mc.transpose.savebin([fpath{f} '/mc' num2str(vs,'%02d') '.bin'])

        mcTrace=xyField; % Normcorre
        save([fpath{f} '/mcTrace' num2str(vs,'%02d') '.mat'],'mcTrace','ave_im')
        fprintf('Motion correction done %2.0f out of %2f.0 movie\n', vs,length(VmovTimesegments)-1)
    end
    % 
