...
%%
mov_mc=readBinMov_BHL_multiple(fpath{f},3,[50000:60000],15000);
mov_res=mov_mc-mean(mov_mc,3);
figure(11); clf;
lag1img=sqrt(mean(mov_res(:,:,2:end).*mov_res(:,:,1:end-1),3));
imshow2(lag1img,[0 300]);
...
for f=[20]%:length(fpath)
    ...
    sz=double(Device_Data{1, 3}.ROI([2 4]));
    ref_time=[2000:3000];
    mov_test=double(readBinMov_times([fpath{f} '/mc_ShutterReg' num2str(3,'%02d') '.bin'],sz(2),sz(1),ref_time));
    ...
    mov_mc=double(readBinMov([fpath{f} '/mc_ShutterReg' num2str(4,'%02d') '.bin'],sz(2),sz(1)));
    ...
    for n=1:size(Result.traces,1)
        ...
        if n==ref_trace %compensate spike hight due to bleaching
            for t=1:length(tN)-1
                ...
                [sp_temp, pks, prom]=find_spike_bh(tr_tmp_norm,7,6);
