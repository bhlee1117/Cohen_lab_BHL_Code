...
mov_mc=double(readBinMov_times([fpath{i} '/mc.bin'],sz2,sz1,[600:900])); %Sq_camera
...
Result{i}.c_ftprnt=mask_footprint(Result{i}.centers,movmean(mov_res(:,:,1000:end),10,3),[],8);
...
Result{i}.traces_bin=-(tovec(mov_res)'*tovec(Result{i}.c_ftprnt>0))';
...
Result{i}.traces_res=squeeze(SeeResiduals(reshape(Result{i}.traces,ns,1,[]),Result{i}.mcTrace'));
    ...
    Result{i}.spike=[];
    tmp=squeeze(Result{i}.traces_res) - movmedian(squeeze(Result{i}.traces_res),4,2); tmp=tmp./get_threshold(tmp,1);
...
%%
figure; i=1;
scale=15; t=[1:size(Result{i}.traces_hi,2)]/frm_rate;
...
%%
wind=[-50:100];
CS_STA=zeros(size(c_ftprnt,3),length(wind));
g=1;
CS_list=[];
for i=18%:size(c_ftprnt,3)
    [list, CS]=find_CS(tr(i,:),spike(i,:),15,5);
    CS_list=[CS_list; list];
    for j=1:size(list,1)
        try
    CS_STA(i,:)=CS_STA(i,:)+tr(i,list(j,1)+wind);
    g=g+1;
        end
    end
    CS_STA(i,:)=CS_STA(i,:)/(g-1);
end

%%
wind=[-50:100];
SS_STA=zeros(size(c_ftprnt,3),length(wind));
CS_list=[];
for i=1:size(c_ftprnt,3)
    ...
    for j=1:length(s)
    try
    SS_STA(i,:)=SS_STA(i,:)+tr(i,s(j)+wind);
    g=g+1;
    end
    end
    ns(i)=g-1;
    SS_STA(i,:)=SS_STA(i,:)/ns(i);
end
