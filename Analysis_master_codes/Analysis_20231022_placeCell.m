clear
clc;
sourcePath='/Volumes/BHL_WD18TB/20231021_BHLm079_PC_DMDs';
cd(sourcePath)
...
for i=1:length(fpath)
    ...
    for n=1:size(Result{i}.centers,1)
        ...
        %Cellpsf = fspecial('gaussian', 2*bs(n)+1, 12);
        Cellpsf=ones(2*bs(n)+1,2*bs(n)+1);
...
save(fullfile(sourcePath,'20231022_Result.mat'),'Result','fpath','-v7.3')
%%
load(fullfile(sourcePath,'20231022_Result.mat'))
...
nexttile([1 1])
plot(rescale2(reshape(Result{g}.normTraces(n,stim_Start+repmat(wnd,4,1)),4,[]),2)'-[1:4]*1.5);
hold all
plot(rescale2(reshape(Result{g}.VR(12,stim_Start+repmat(wnd,4,1)),4,[]),2)'-[1:4]*1.5,'color',[0 0.5 1]);
axis off
...
legend({'Stim Sq#1','Stim Sq#2','Stim Ps#1','Stim Ps#2'},'Location','northwestoutside')
