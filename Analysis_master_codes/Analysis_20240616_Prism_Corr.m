...
nTau_bAP=[-20:20];
nTau={[-50:15],[-50:150],[-50:20]}; %SS, CS, dSP
...
%NomalizedTrace_ch=cellfun(@(x) x./F_ref,Result.norm_trace_check,'UniformOutput',false);
...
%STA_SSmat_ch=cellfun(@(x) reshape(NomalizedTrace(:,bAP_s'+nTau{1}),nROI,[],length(nTau{1})),Result.norm_trace_check,'UniformOutput',false);
%STA_dSPmat_ch=cellfun(@(x) reshape(NomalizedTrace(:,dSP_s'+nTau{3}),nROI,[],length(nTau{3})),Result.norm_trace_check,'UniformOutput',false);
...
%%
presp_time=54; bin=3; noi=setdiff([1:nROI],[9]);
SS_presub=permute(STA_SSmat(dist_order(noi),:,-nTau{1}(1)-presp_time:-nTau{1}(1)-1),[1 3 2]);
SS_presub=movmean(SS_presub,bin,2,'omitnan');
SS_presub=SS_presub(:,[1:bin:end],:);
SS_presub=tovec(SS_presub); SS_presub=SS_presub-mean(SS_presub,2);

CS_presub=permute(STA_CSmat(dist_order(noi),:,-nTau{2}(1)-presp_time:-nTau{2}(1)-1),[1 3 2]);
CS_presub=movmean(CS_presub,bin,2,'omitnan');
CS_presub=CS_presub(:,[1:bin:end],:);
CS_presub=tovec(CS_presub); CS_presub=CS_presub-mean(CS_presub,2);

covMat = SS_presub*SS_presub';
[V, D] = eig(covMat);
D = diag(D);
D = D(end:-1:1);
V = V(:,end:-1:1);
...
SilentTrace2=NomalizedTrace_ch;
for ch=1:length(SilentTrace)
