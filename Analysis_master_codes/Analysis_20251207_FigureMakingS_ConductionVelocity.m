...
figure(105); tiledlayout('TileSpacing','compact'); clf;
    ...
    for stype=1:3
        for s=1:numsp2show(stype)
            ...
            if ~isempty(MatSTA{f,stype,s})
                ...
                nexttile([1 1])
                scatter(xaxis(ap),SpikeDelay{f,stype}(ap,s),'filled'); hold all
                plot(xaxis(ap),y_fit)
                title([num2str(f), ', s=' num2str(s)])
...
pval=get_pValue(conduction_Speed,1);
...
%drawPValueLines(pval,0,'TextYOffset',20,'StepHeight',20)
