...
str=[[transformPointsForward(RegT,[tree.X tree.Y])] tree.D/mean([sx sy])];
else
    str=[tree.X tree.Y tree.D];
end
somacoord=get_coord(Result.ftprnt(:,:,1));
offset=str(1,[1 2])-somacoord-[0 0];
str(:,[1 2])=str(:,[1 2])-offset;

figure(19); clf; %show footprint
imshow2(Result.ref_im,[]); hold all;
scatter(str(:,1),str(:,2),str(:,3)+5,[0.6 0.6 0.6],'filled'); axis equal tight off; 
for r=1:nROI
DMDmask=ftprnts(:,:,r)>0;
...
scatter(str_ROI(:,1),str_ROI(:,2),str_ROI(:,3)+5,cmap_ROIs(r,:),'filled');
