...
clear
fpath='/Volumes/cohen_lab/Lab/Labmembers/Byung Hun Lee/Data/20220627_Treadmill/013646_20220626_M2_Treadmill_Stim_p4/';
%fname='Sq_camera.bin';
fname=['motion_corrected/mc' num2str(4,'%02d') '.bin'];
mov=double(readBinMov_times(fullfile(fpath,fname),304,512,[1:10000]));
%mov=double(vm(fpath));
moviefixsc(mov)

%%
mov_test=vm(mov(:,:,150:250)); % Reference to motion correction
mov_test = single(mov_test)./single(max(mov_test.data(:)));
mov_test = movmean(mov_test,10,3);
mov_ref = squeeze(median(mov_test,3));


[mov_mc,xyField]=optical_flow_motion_correction_LBH(vm(mov),mov_ref); % Motion correction
...
centers=cell_detection_manual(mean(mov_mc,3),centers); %manually add or discard
%%
rad=5;
...
mov_res_vec = tovec(mov_res(:,:,idx_seg));
...
c_ftprnt = reshape(c_ftprnt,size(mov_mc,1),size(mov_mc,2),[]);

save('20220803_mask.mat','c_ftprnt','traces')

%%
n=squeeze(sum(sum(c_ftprnt>0,1),2));
idx_t = 1:size(traces,2)-100;
...
mov_bin_s=reshape(a,256,256,[]);%./max(mov_gauss,[],3);
%%
...
mov_xcorr=toimg(tovec(mov_bin_s2(:,:,101:3000)).*tovec(mean(mov_mc,3)),304,512);
...
[V_pc, D_pc,U_pc] = svds(double(tovec(mov_xcorr(:,:,1:2900))),n_pc);
...
for ii = 1:16
    subplot(4,4,ii)
    imshow2(toimg(V_pc(:,idx_pc(ii)),size(mov_xcorr,1),size(mov_xcorr,2)),[prctile(V_pc(:,idx_pc(ii)),1) prctile(V_pc(:,idx_pc(ii)),99)])
    title(num2str(ii))
end

%%
idx_pc = [1:3];
[icasig, A,W] = fastica((V_pc(:,idx_pc)'*tovec(mov_xcorr)));
n_ic = size(W,1);
V_ic = (W * V_pc(:,idx_pc)')';
ic_sub = (V_ic(:,1:n_ic)'*tovec(mov_xcorr))';
mov_res = SeeResiduals(mov_res(:,:,1:2900),ic_sub);
%%
