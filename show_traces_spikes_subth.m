function [ax1]=show_traces_spikes_subth(traces,spikes,sub,otherT,frmrate)
if nargin<5
    frmrate=1;
end
%figure;
...
%arrayfun(@(l,c) set(l,'Color',c{:}),lines,num2cell(line_color,2))
hold all
linesub=plot(t,sub(noi,:)'+[1:size(noi,2)]*scale,'linewidth',2);
arrayfun(@(l,c) set(l,'Color',c{:}),linesub,num2cell(1-line_color,2))
S=tr; S(~(spikes==1))=NaN;
